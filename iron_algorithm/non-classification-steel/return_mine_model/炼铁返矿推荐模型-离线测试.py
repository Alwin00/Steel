...
def hitrate(y_test, y_predict, level1=0.2, level2=0.4, level3=0.6):
    '''
    输入：
    y_test,实际值
    y_predict,预测值
...
def cal_LTFK(name, mean, scale):
    ...
    hit_rate1, hit_rate2, hit_rate3, MAE = hitrate(y_test, answer,  level1=0.2, level2=0.4, level3=0.6)
    print(name[0:-5]+'炼铁返矿命中率:', hit_rate1, hit_rate2, hit_rate3, MAE)
...
def main():
    ...
    for fname in os.listdir('.'):
        if  fname.find(f'返矿') >= 0 and fname.find(f'xlsx') > 0:
            ...
            # print(fname[:-5])
            y_predict = cal_LTFK(fname, df_std.loc[fname[:-5], 'outmean'], df_std.loc[fname[:-5], 'outscale'])
