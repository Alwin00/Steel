    ...
    for fname in os.listdir('../../..'):
        ...
        if fname.find(f'氧气') >= 0 and fname.find(f'xlsx') > 0:
            ...
            for i in range(len(x_scale.mean_)):
                df.loc[fname[0:-5], 'inmean' + str(i)] = x_scale.mean_[i]
                df.loc[fname[0:-5], 'inscale' + str(i)] = x_scale.scale_[i]
            df.loc[fname[0:-5], 'outmean'] = y_scale.mean_[0]
            df.loc[fname[0:-5], 'outscale'] = y_scale.scale_[0]
    return df


if __name__ == '__main__':
    df = std()
    with pd.ExcelWriter('std.xlsx') as writer:
        df.to_excel(writer, sheet_name='std')
