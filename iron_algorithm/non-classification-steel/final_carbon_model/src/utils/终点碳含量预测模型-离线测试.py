...
def hitrate(y_test, y_predict, level1=0.015, level2=0.02, level3=0.025):
    '''
    输入：
    y_test,实际值
    y_predict,预测值
...
def cal_O2(name, mean, scale):
    ...
    X_test = data1[predict_C].values
    y_test = data1[y_C].values
    # 输入数据标准化
    ...
    hit_rate1, hit_rate2, hit_rate3, MAE = hitrate(y_test, answer, level1=0.015, level2=0.02, level3=0.025)
    print(name[0:-5]+'终点碳含量预测命中率:', hit_rate1, hit_rate2, hit_rate3, MAE)
...
def main():
    ...
    for fname in os.listdir('../..'):
        if  fname.find(f'碳含量') >= 0 and fname.find(f'xlsx') > 0:
