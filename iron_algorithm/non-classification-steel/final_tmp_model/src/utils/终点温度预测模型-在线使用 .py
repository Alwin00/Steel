...
def JUDGE(data):
    if 69.89<=data['MOLTIRON_WT'].values<=100.35 and 1224<=data['IRON_TEMP'].values<=1413 and 3.55<=data['iron_c'].values<=5.25 and\
            0.1<=data['iron_si'].values<=0.83 and 6.0<=data['5G11'].values<=28.0 and 3290<=data['O2_SUM_COMSUME'].values<=5095 and 0.91<=data['8ZCSSH'].values<=5.42 :
        judge = True
        return judge
    else:
        judge = False
...
def mainpredict(data):
    ...
    judge = JUDGE(data)
    if judge == True:
       answer=BOF_END_T_DATA('不分钢种终点温度_1630-1700',data)
       return answer
